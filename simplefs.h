...
/* 
   This function will be used to create a virtual disk (a Linux file)
...
/*
  This function is will be used to initialize/create (high level format) 
  the sfs file system on the virtual disk. On disk structures will 
  be initialized as part of this call. 
...
/* 
   ...
   Basically, it will open the regular Linux file and obtain 
   a file descriptor.  Other operations will use this
   file descriptor in the library implementation. This virtual 
   disk (Linux file) descriptor will be a global variable in
   the library. 
   If success, 0 will be returned; if error, -1 will be returned. 
 */ 
...
/*
   ...
   to open is filename. The mode paramater specifies if the 
   file will be opened in read-only mode or in append-only mode. 
   if 0, read-only; if 1, append-only. We can just 
   read file or append to it. A file can not be opened for both
   reading and appending at the same time. 
   The return value will be a non-negative integer acting as a
...
/* 
   ...
   fd is the file descriptor. buf is pointing to a static array 
   holding the data or a dynamically allocted memory space holding
   the data. n is the size of data to write into the file. 
   Upon failure, will return -1. Otherwise, the number of bytes
...
/* 
   With this, an application can delet a file. The name of the
   file to be deleted is filename. In succesful, 0 will be returned. 
